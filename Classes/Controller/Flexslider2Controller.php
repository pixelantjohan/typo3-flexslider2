...
namespace B263\Fs2\Controller;

use \TYPO3\CMS\Extbase\Mvc\Controller\ActionController;

class Flexslider2Controller extends ActionController {
	...
	protected function insertResources() {
		$resources = array(
			'addCssFile' => array('cssLibFile'),
			'addJsFooterFile' => array('jsLibFile', 'jsFile'),
		);
		foreach ($resources as $method => $settingsKeyList) {
			foreach ($settingsKeyList as $settingsKey) {
				if (isset($this->settings[$settingsKey]) && ! empty($this->settings[$settingsKey])) {
					$GLOBALS['TSFE']->getPageRenderer()->$method($GLOBALS['TSFE']->tmpl->getFileName($this->settings[$settingsKey]));
				}
			}
		}
	}
